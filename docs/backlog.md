...
| 항목 | 조건 | 참고 |
|---|---|---|
| **AI 리뷰 도구 연동** (CodeRabbit 등) | PR 루프·리뷰 대응 규칙(AGENTS.md 10장, create-pr 스킬)은 이미 있다. 도구만 붙이면 된다 | — |
| **FE 테스트 도입 + 커버리지 게이트** | 프레임워크(vitest)부터 없다. FE 에 로직이 생길 때 한 묶음으로 | BE 게이트는 100%로 걸려 있음 (`ci.yml`) |
| **이벤트 봉투 + Transactional Outbox** | 이벤트·메시징 코드가 생길 때. 봉투 5필드(eventId=멱등 열쇠, aggregateId=순서 키, 시각은 봉투에만), 저장과 발송을 한 트랜잭션에 섞지 않기 | team-pulse `docs/이벤트토픽.md` (팀 미확정 상태) |
| **인가 패턴** | 인증·권한 코드가 생길 때. 권한 없으면 403 이 아니라 404(존재를 알려주지 않는다), 권한 판단은 컨트롤러가 아니라 질의 조건에 | property-right review 스킬 |
...
| 자율 파이프라인 완성 (3차 조사 반영) | 커밋 게이트 제거(입구=스펙 승인·출구=머지만 사람) · PR 후 CI/리뷰 따라가기 · BE 커버리지 100% 게이트 · specs 6개월 보관 규칙 | `specs/2026-08-30-autonomous-pipeline/` |
